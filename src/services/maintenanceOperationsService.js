...
export const maintenanceOperationsApi = {
  ...
  assignWorkOrder: (auth, id, data) => request(auth, `/work-orders/${id}/assign`, { method: 'POST', body: JSON.stringify(data) }),  acceptWorkOrder: (auth, id, data = {}) => request(auth, `/work-orders/${id}/accept`, { method: 'POST', body: JSON.stringify(data) }),
  startWorkOrder: (auth, id, data = {}) => request(auth, `/work-orders/${id}/start`, { method: 'POST', body: JSON.stringify(data) }),
  saveFieldReport: (auth, id, data) => request(auth, `/work-orders/${id}/field-report`, { method: 'PUT', body: JSON.stringify(data) }),
  addWorkOrderEvidence: (auth, id, data) => request(auth, `/work-orders/${id}/evidence`, { method: 'POST', body: JSON.stringify(data) }),
  completeWorkOrder: (auth, id, data = {}) => request(auth, `/work-orders/${id}/complete`, { method: 'POST', body: JSON.stringify(data) }),
