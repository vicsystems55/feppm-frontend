      ...
      <main class="maintenance-page">
        ...
        <section v-else-if="activeTab === 'requests'" class="maintenance-panel">
          ...
          <div class="maintenance-table-wrap"><table><thead><tr><th>Request</th><th>Facility / equipment</th><th>Priority</th><th>Triage</th><th>Status</th><th /></tr></thead><tbody>
            <tr v-for="ticket in requests" :key="ticket.id"><td><strong>{{ ticket.ticketNumber }}</strong><span>{{ ticket.title }}</span><small>{{ date(ticket.reportedAt) }}</small></td><td><strong>{{ ticket.facility?.name ?? 'No facility' }}</strong><span>{{ ticket.equipment?.assetCode ?? 'No equipment selected' }}</span></td><td><b :class="`priority p${ticket.priority}`">P{{ ticket.priority }}</b></td><td><span :class="['triage-state', { done: ticket.triage }]">{{ ticket.triage ? label(ticket.triage.decision) : 'Awaiting triage' }}</span></td><td>{{ label(ticket.status) }}</td><td><button class="link-button" type="button" @click="openRequest(ticket)">Review</button></td></tr>
