...
async function loadEquipment(page = 1) {
  ...
  errorMessage.value = '';
  try {
    const response = await auth.authorizedFetch(`/equipment?${query(page)}`);
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(payload.message ?? 'Unable to load equipment registry.');
...
async function viewEquipment(item) {
  ...
  try {
    const response = await auth.authorizedFetch(`/equipment/${item.id}`);
    const payload = await response.json().catch(() => ({}));
...
async function loadHierarchy() {
  try {
    const response = await auth.authorizedFetch('/facilities/tree');
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(payload.message ?? 'Unable to load facility hierarchy.');
    treeNodes.value = payload.data.tree;
  } catch (error) {
    errorMessage.value = error.message;
  }
}
...
function locationPath(facility) {
  const parts = [];
  let unit = facility?.administrativeUnit;
  while (unit) {
    parts.push(unit.name);
    unit = unit.parent;
  }
  return parts.reverse().join(' / ') || 'Administrative location not assigned';
}
...
onMounted(() => Promise.all([loadEquipment(), loadHierarchy()]));
</script>
    ...
    <div class="dashboard-main">
      ...
      <main class="lga-equipment-page">
        <header class="equipment-heading">
          <div><p>Scoped equipment registry</p><h1>Equipment across {{ scopeName }}</h1><span>Browse equipment through the national, state, LGA and facility hierarchy available to your account.</span></div>
        ...
        <div class="equipment-workspace">
          <aside class="equipment-hierarchy">
            <FacilityTree
              v-if="treeNodes.length"
              :nodes="treeNodes"
              @select="selectFacility"
            />
            <div v-else class="equipment-hierarchy__empty"><Building2 :size="25" /><span>No facility hierarchy is available in this scope.</span></div>
          </aside>
          ...
          <div v-else-if="equipment.length" class="equipment-table-wrap">
            <table class="equipment-table">
              <thead><tr><th>Asset</th><th>Facility hierarchy</th><th>Equipment type</th><th>Model</th><th>Functionality</th><th>Condition</th><th>Schedules</th><th><span class="sr-only">Action</span></th></tr></thead>
              <tbody>
                <tr v-for="item in equipment" :key="item.id">
                  <td><strong>{{ item.assetCode }}</strong><small>{{ item.serialNumber || 'No serial number' }}</small></td>
                  <td><strong>{{ item.facility.name }}</strong><small>{{ locationPath(item.facility) }}</small></td>
...
.equipment-workspace{display:grid;grid-template-columns:minmax(280px,340px) minmax(0,1fr);align-items:start;gap:14px}.equipment-hierarchy{max-height:720px;overflow:auto;border:1px solid var(--border);border-radius:12px;background:#fff}.equipment-hierarchy__empty{min-height:220px;padding:24px;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:9px;color:#667085;text-align:center;font-size:11px}.equipment-hierarchy__empty svg{color:#98a2b3}
